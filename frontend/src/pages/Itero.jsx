...
export default function Itero() {
  ...
  useEffect(() => {
    setData(null);
    const url = showManagerView ? "/dashboard/manager/itero" : "/dashboard/tm/itero";
  ...
  return (
    <div data-testid="itero-page">
      <div className="mb-6 flex items-start gap-3">
        ...
        <div>
          <div className="text-xs uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>{showManagerView ? "iTero (manager view)" : "iTero (TM view)"}</div>
      ...
      {isSeniorTM && (
        <div
          ...
          data-testid="seniortm-itero-view-toggle"
          ...
          {[
            ...
            return (
              <button
                ...
                data-testid={`seniortm-itero-view-${opt.key}`}
                aria-pressed={active}
      ...
      {data && (showManagerView ? <ManagerItero data={data} /> : <TMItero data={data} />)}
    </div>
  );
}
